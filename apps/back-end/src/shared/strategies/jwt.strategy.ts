...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ///Esta clase va a extender (heredar) de PassportStrategy y le pasamos por parametro el patron
  constructor(configService: ConfigService) {
    /// inyectamos por constructor el configService para traer nuestra firma de los JWT
  ...
  async validate(payload: any) {
    ///Validamos el payload del JWT
    return { userId: payload.sub, email: payload.mail, role: payload.role }; ///Devolvemos los datos que queremos que esten en req.user
  }
}
