...
export class GoogleAuthService {
    ...
    async handleGoogleCallback(code : string) : Promise<ApiResponse<GoogleResponseDTO>>{ ///esperamos el codigo que va a llegar por la URL que nos proporciona google
        ...
        const tokenResponse = await firstValueFrom(
            this.httpService.post( ///hacemos una peticion HTTP post a la API de google
                ...
                { headers: { 'Content-Type': 'application/json' } } /// y los headers principales para la peticion
            ),
        );
        ...
        if(userexists){ // si es true verificamos si OAuth es true
            if(userexists.isOauthUser){ /// si es true creamos los tokens y retornamos
                ...
                await this.refreshTokenRepository.replaceRefreshToken(refreshToken, expires, userexists.id)
        ...
        }else { /// si el usuario no existe con el email retornado por google, creamos el usuario y la persona (los datos faltantes los podemos pedir cuando este en el dashboard)
            const role = await this.roleService.findByName('PROFESSIONAL')
            const { user } = await this.prismaService.$transaction(async () => {
                const person = await this.personRepository.createPerson(googleDTO.name, googleDTO.lastName,"")
                const user = await this.userRepository.createUser("", googleDTO.email,googleDTO.pictureUrl, "",role.id, person.id,true)
            ...
            await this.refreshTokenRepository.createRefreshToken(refreshToken, expires, user.id) // guardamos el refresh token en la DB

            const data : GoogleResponseDTO = { ///creamos el data a devolver
                    message: 'inicio de sesion exitoso',
                    accessToken: accessToken,
                    refreshToken: refreshToken
                }
