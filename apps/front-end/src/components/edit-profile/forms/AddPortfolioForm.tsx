...
export const AddPortfolioForm = ({ userFields, onSave, onCancel, isLoading }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title || !fieldId || files.length === 0) {
      alert("Completa el título, selecciona un rubro y agrega al menos una foto.");
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Rubro del trabajo</label>
        <select
          value={fieldId}
        ...
        <div className="grid grid-cols-3 gap-4">
          {previews.map((src, index) => (
            <div key={index} className="relative aspect-square rounded-xl overflow-hidden border">
              ...
              <button type="button" onClick={() => removeFile(index)} className="absolute top-1 right-1 bg-white/80 p-1 rounded-full text-red-500 hover:bg-white">
                <X size={14} />
              </button>
            </div>
          ))}
      ...
      <div className="flex gap-3 pt-4 border-t border-gray-100">
        <button type="button" onClick={onCancel} disabled={isLoading} className="flex-1 py-3 bg-gray-100 rounded-xl">Cancelar</button>
        <button type="submit" disabled={isLoading} className="flex-1 py-3 bg-black text-white rounded-xl hover:bg-gray-800 disabled:opacity-50">
