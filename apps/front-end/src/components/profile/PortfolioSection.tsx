...
export const PortfolioSection = ({ items, isLoading }: Props) => {
  ...
  // Skeleton Loading (Opcional, pero recomendado)
  if (isLoading) {
    return (
  ...
  // Si no hay items, no mostramos nada (o un mensaje vacío)
    ...
    <div className="mt-12 border-t border-gray-200 pt-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {items.map((item) => {
          ...
          return (
            <div key={item.id} className="group relative bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300">
              {/* Imagen */}
              <div className="relative aspect-4/3 overflow-hidden bg-gray-100">
                {coverImage ? (
                  ...
                  // Placeholder si el proyecto no tiene fotos
