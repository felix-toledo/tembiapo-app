...
export default async function ProfilePage(props: {
  ...
  const mainField =
    profile.fields.find((f) => f.isMain)?.name ||
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {profile.area.map((a: any) => (
                      <span
                        key={a.id}
                        className="px-3 py-1 bg-gray-100 rounded-lg text-sm font-medium text-gray-700 border border-gray-200"
                      >
                        {a.city}
                      </span>
                    ))}
              ...
              {profile.portfolio.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {profile.portfolio.map((item) => (
                    <div
                      key={item.id}
                      className="group relative rounded-2xl overflow-hidden border-2 border-gray-100 shadow-sm hover:shadow-md transition-all"
                    >
